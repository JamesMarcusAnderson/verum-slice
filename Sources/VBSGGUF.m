...
- (instancetype)initWithPath:(NSString *)path error:(NSError **)error {
    ...
    struct stat st;if(fstat(_fd,&st)||st.st_size<24){if(error)*error=err(@"Invalid GGUF size");close(_fd);return nil;}_fileSize=(uint64_t)st.st_size;
    _map=mmap(NULL,(size_t)_fileSize,PROT_READ,MAP_PRIVATE,_fd,0);if(_map==MAP_FAILED){if(error)*error=err(@"mmap failed");close(_fd);return nil;}
